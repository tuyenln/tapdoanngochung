...
function product_category() {
	$name = get_query_var('product_cat');
	// $a = get_cat_ID( $id );
	...
	// <a href="' . $link . '" class="m-btn">Xem thêm</a>
	return array($len, $output);
}
list ($len, $product_category) = product_category();


// add_action( 'woocommerce_before_main_content', 'product_category', 10, 2 );
			...
			<?php   if ( apply_filters( 'woocommerce_show_page_title', true ) ) : ?>
				...
				*/
				...
				<div class="col-xs-12">
					<div class="m-header">
						<div>
							<h3><span><?php is_shop()  ? print('SẢN PHẨM') : woocommerce_page_title(); ?></span></h3>
						</div>
					</div>
				</div>
